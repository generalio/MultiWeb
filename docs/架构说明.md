...
- `ScriptBridgeOriginPolicy.UnsafeAnyHttpOrHttps` 是迁移旧网页时才可显式选择的高风险桥策略。iOS 与 Desktop 仅允许
  HTTP/HTTPS 主框架，仍拒绝 `file:`、`data:`、自定义 Scheme 与子框架；AndroidX WebKit 无法表达同等约束，因此
  Android 会拒绝安装该策略，JS/Wasm 也不会安装桥。
