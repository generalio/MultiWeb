...
`webview-ios`、`webview-desktop`、`webview-browser` 和 `webview-compose`。`webview-test-fixtures` 是工程内部测试夹具，
...
1. 日常开发版本由 [自动推进开发版本工作流](../.github/workflows/promote-development-version.yml) 维护。每次正式发布完成后，工作流会把
   `VERSION_NAME` 推进到该标签的下一个 patch `-SNAPSHOT` 版本；例如 `v0.2.0` 成功发布后变为
   `0.2.1-SNAPSHOT`。
...
4. GitHub Packages、Maven Central 与 GitHub Release 均成功后，自动推进开发版本工作流会直接提交下一个
   patch `-SNAPSHOT` 版本，并通过 `workflow_dispatch` 触发主分支构建校验；无需额外创建版本推进 PR。
...
# GitHub Packages 仓库按环境变量条件注册；占位值只用于发现任务，不会上传构件。
GITHUB_REPOSITORY=owner/repository GITHUB_ACTOR=local GITHUB_TOKEN=placeholder \
  ./gradlew tasks --all | rg "publishAllPublicationsToGitHubPackagesRepository"
```

第一段命令退出为 `0` 即表示测试夹具不存在 Maven 发布任务。Kotlin 可能显示
`export*PublicationCoordinates*` 元数据任务，它们不会发布或上传构件。第二条命令模拟 GitHub Actions
注册 GitHub Packages 仓库，仅用于确认正式发布入口仍可被 Gradle 发现。
